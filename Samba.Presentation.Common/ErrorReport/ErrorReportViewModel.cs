    ...
    class ErrorReportViewModel : ObservableObject
    {
        ...
        public void SubmitError()
        {
            ...
            string queryString = string.Format("from={0}&emaila={1}&file={2}",
                Uri.EscapeDataString("info@allagma.com"),
                Uri.EscapeDataString("SambaPOSSE V3 Error Report"),
                Uri.EscapeDataString(tempFile));

            var c = new WebClient();
            var result = c.UploadFile("http://reports.allagma.com/file.php?" + queryString, "POST", tempFile);
