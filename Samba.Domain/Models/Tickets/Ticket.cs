...
namespace Samba.Domain.Models.Tickets
{
    public class Ticket : EntityClass, ICacheable
    {
        ...
        public Ticket()
            : this(0)
        {
            SRSTransactionNo = TicketNumber;   //Added by Tim GU
        }
        ...
        public int SRSSubmitCount { get; set; } = 0;  //Added by Tim GU
        public int SRSSubNo { get; set; } = 0;  //Added by Tim GU
        public string SRSTransactionNo { get; set; }  //Added by Tim GU
        public string RefSRSTransactionNo { get; set; }  //Added by Tim GU
        ...
        public bool IsReopened { get; set; } = false;      //Added by Tim GU

        public void Reopen()    //Added by Tim GU
        {
            IsReopened = true;
            IsClosed = false;
            UnLock();
            Payments.ToList().ForEach(RemovePayment);              //TBD, shall add codes here to call real reverse payment modules such as Monaris SDK method, to handle the reverse payment procedure accordingly
            ChangePayments.ToList().ForEach(RemoveChangePayment);
            PaidItems.Clear();
            RemainingAmount = TotalAmount;
            foreach (Order order in Orders)
                order.SetStateValue("Status", 1, "Reopened", 1, "", 0);
            SetStateValue("Status", "Reopened", "");
        }

        public bool IsRefund { get; set; } = false;      //Added by Tim GU
        ...
        public void AddPayment(PaymentType paymentType, Account account, decimal amount, decimal exchangeRate, int userId)
        {
            ...
            //Begin of the code blocks added by Tim GU
            decimal versActu = 0;
            decimal versAnt = 0;
            decimal sold = 0;
            ...
            else
            {
                if(Payments.Count == 0)   //The first instalment of a split payment
                {
                    versActu = amount;
                    versAnt = 0;
                    sold = RemainingAmount - amount;
                }
                else    //A middle instalment of a split payment
                {
                    versActu = amount;
                    if (Payments.Count > 0)
                    {
            ...
            //End of the code blocks added by Tim GU

            var payment = new Payment { AccountTransaction = transaction, Amount = amount, Name = account.Name, PaymentTypeId = paymentType.Id, UserId = userId, VersActu = versActu, VersAnt = versAnt, Sold = sold };  //Modified by Tim GU
        ...
        public void RemoveAllChangePayments()
        {
            if (ChangePayments.Count > 0)
            {
                for (int i = ChangePayments.Count - 1; i >= 0; i--)
                    RemoveChangePayment(ChangePayments[i]);
            }
        }
        ...
        public int GetCustomerCount()   //Added by Tim GU
        {
            ...
            {
                IList<TicketTagValue> TicketTagValues = JsonHelper.Deserialize<List<TicketTagValue>>(TicketTags);
                if (TicketTagValues.Count > 0)
                {
                    for (int i = 0; i < TicketTagValues.Count; i++)
                    {
                        if (TicketTagValues[i].TagName == "Nombre de Clients")
                        {
                            try
                            {
                                iRet = Convert.ToInt32(TicketTagValues[i].TagValue);
                            }
                            catch (Exception) { };
                            break;
                        }
        ...
        {
            var taxId = 0;
            if (GetTaxIdByTaxName("TPS").Count() > 0)
                taxId = GetTaxIdByTaxName("TPS")[0];
            var result = Orders.Where(x => x.CalculatePrice).Sum(x => x.GetTotalTaxAmount(TaxIncluded, plainSum, preTaxServices, taxId));
            return decimal.Round(result, LocalSettings.Decimals, MidpointRounding.AwayFromZero);
        }

        public decimal CalculateTVQ(decimal plainSum, decimal preTaxServices)
        {
            var taxId = 0;
            if (GetTaxIdByTaxName("TVQ").Count() > 0)
                taxId = GetTaxIdByTaxName("TVQ")[0];
            var result = Orders.Where(x => x.CalculatePrice).Sum(x => x.GetTotalTaxAmount(TaxIncluded, plainSum, preTaxServices, taxId));
            return decimal.Round(result, LocalSettings.Decimals, MidpointRounding.AwayFromZero);
        }
        ...
        {           
            ...
            foreach (var calculation in calculations.OrderBy(x => x.Order))
            {
                ...
                if(calculation.Name == "Pourboire %")   //Added by Tim GU
                {
                    ...
                    for(int i =0; i < Calculations.Count; i++)
                    {
                        ...
                        {
                            discountPercent = Calculations[i].Amount;
                            break;
                        }
                    }
                ...
                //if (calculation.Amount == 0 && calculation.CalculationType != 5)    //Commented by Tim GU
                if ((calculation.Amount == 0 && calculation.CalculationType != 5) || (calculation.Name == "Pourboire %" && calculation.Amount > 25))   //Added by Tim GU
                {
            ...
            return decimal.Round(totalAmount, LocalSettings.Decimals, MidpointRounding.AwayFromZero);  //Modified by Tim GU, added the MidpointRounding type
        }
        ...
        public decimal GetRemainingAmount()
        {
            ...
            return decimal.Round(sum - payment + changePayment, LocalSettings.Decimals, MidpointRounding.AwayFromZero); //Modified by Tim GU, added the MidpointRounding type
        }
        ...
        public IList<int> GetTaxIdByTaxName(string TaxName)
        {
            return Orders.SelectMany(x => x.TaxValues)
                            .Where(x => x.TaxTempleteAccountTransactionTypeId > 0 && x.TaxTemplateName == TaxName)
                            .Select(x => x.TaxTempleteAccountTransactionTypeId)
                               .Distinct().ToList();
        }
        ...
        public void Recalculate()
        {
            if (Orders.Count > 0)
            {
                ...
                if (bIsCancelled)
                {
                    for (int i = Calculations.Count - 1; i >= 0; i--)
                    {
                        if (!Calculations[i].Name.Contains("Annulation"))
            ...
            else    //Added by Tim GU.   To reflect the situation when NO orders placed, all exsiting calculations such as Tips and Discount should be cleared.
            {
                if (Calculations.Count > 0)
                    Calculations.Clear();
            }
        ...
        public decimal GetBeforeTax()    //Added by Tim GU
        {
            var result = GetPlainSum() + GetPreTaxServicesTotal();
        ...
        public decimal GetTPSTotal()    //Added by Tim GU
        {
            var taxId = GetTaxIdByTaxName("TPS")[0];

            var result = Orders.Sum(x => x.GetTotalTaxAmount(TaxIncluded, GetPlainSum(), GetPreTaxServicesTotal(), taxId));
            return decimal.Round(result, 2, MidpointRounding.AwayFromZero);
        }

        public decimal GetTVQTotal()    //Added by Tim GU
        {
            var taxId = GetTaxIdByTaxName("TVQ")[0];

            var result = Orders.Sum(x => x.GetTotalTaxAmount(TaxIncluded, GetPlainSum(), GetPreTaxServicesTotal(), taxId));
            return decimal.Round(result, 2, MidpointRounding.AwayFromZero);
        }

        public decimal GetAfterTax()    //Added by Tim GU
        {
            var result = GetBeforeTax() + GetTPSTotal() + GetTVQTotal();
            return decimal.Round(result, 2, MidpointRounding.AwayFromZero);
        }
