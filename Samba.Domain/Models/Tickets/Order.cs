...
namespace Samba.Domain.Models.Tickets
{
    public class Order : ValueClass
    {
        ...
        public decimal GetTotalTaxAmount(bool taxIncluded, decimal plainSum, decimal preTaxServices)
        {
            ...
            //var result0 = CalculatePrice ? TaxValues.Where(x => x.TaxTempleteAccountTransactionTypeId == 8)
            //    .Sum(x => x.GetTaxAmount(taxIncluded, GetTaxablePrice(), TaxValues.Sum(y => y.TaxRate), plainSum, preTaxServices)) * Quantity : 0;
            //var result1 = CalculatePrice ? TaxValues.Where(x => x.TaxTempleteAccountTransactionTypeId == 9)
            //    .Sum(x => x.GetTaxAmount(taxIncluded, GetTaxablePrice(), TaxValues.Sum(y => y.TaxRate), plainSum, preTaxServices)) * Quantity : 0;

            //return decimal.Round(result0, 2, MidpointRounding.AwayFromZero) + decimal.Round(result1, 2, MidpointRounding.AwayFromZero);
        }
