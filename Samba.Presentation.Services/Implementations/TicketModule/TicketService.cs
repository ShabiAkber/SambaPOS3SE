...
namespace Samba.Presentation.Services.Implementations.TicketModule
{
    ...
    public class TicketService : ITicketService
    {
        ...
        public TicketCommitResult TicketRefund(Ticket ticket)   //Function added by Tim GU
        {
            //_applicationState.NotifyEvent(RuleEventNames.TicketMoving, new { Ticket = ticket });
            var clonedEntites = ticket.TicketEntities.Select(ObjectCloner.Clone).ToList();
            var clonedOrders = ticket.Orders.Select(ObjectCloner.Clone2).ToList();
            var clonedCalculations = ticket.Calculations.Select(ObjectCloner.Clone2).ToList();
            CloseTicket(ticket);

            //var ticketType = _cacheService.GetTicketTypeById(ticket.TicketTypeId);
            var newTicket = OpenTicket(0);
            //newTicket = ObjectCloner.Clone(ticket);

            foreach (var clonedOrder in clonedOrders)
            {
                clonedOrder.TicketId = 0;
                clonedOrder.Price = -clonedOrder.Price;
                clonedOrder.SetStateValue("Status", 1, "Refund", 1, "", 0);
                newTicket.Orders.Add(clonedOrder);
            }
            clonedEntites.ForEach(x => newTicket.UpdateEntity(x.EntityTypeId, x.EntityId, x.EntityName, x.AccountTypeId, x.AccountId, x.EntityCustomData));
            if (clonedCalculations != null && clonedCalculations.Count > 0)
            {
                foreach( var clonedCalculation in clonedCalculations)
                {
                    clonedCalculation.TicketId = 0;
                    clonedCalculation.DecreaseAmount = !clonedCalculation.DecreaseAmount;
                    //clonedCalculation.CalculationAmount = -clonedCalculation.CalculationAmount;
                    newTicket.Calculations.Add(clonedCalculation);
                }
            }
            RefreshAccountTransactions(newTicket);

            //newTicket.TicketNumber = _settingService.GetNextString(ticketType.TicketNumerator.Id);
            DateTime dt = DateTime.Now;
            newTicket.Date = dt;
            newTicket.LastOrderDate = dt;
            newTicket.LastUpdateTime = dt;
            newTicket.SRSSubmitCount = 0;
            newTicket.SRSSubNo = 0;
            newTicket.SRSTransactionNo = newTicket.TicketNumber;
            newTicket.RefSRSTransactionNo = ticket.SRSTransactionNo;
            newTicket.LastModifiedUserName = ticket.LastModifiedUserName;
            newTicket.IsClosed = false;
            newTicket.IsLocked = false;
            newTicket.IsReopened = false;
            newTicket.IsRefund = true;
            newTicket.SetStateValue("Status", "Refund", "", "");
            newTicket.Recalculate();
            //newTicket.TotalAmount = -ticket.TotalAmount;
            //newTicket.RemainingAmount = -ticket.TotalAmount;

            _ticketDao.Save(newTicket);
        ...
        private Ticket CreateTicket()
        {
            ...
            UpdateTicketNumber(result, _cacheService.GetTicketTypeById(result.TicketTypeId).TicketNumerator);    //Added by Tim GU
            return result;
        }

        public void ForcedCloseTicket(Ticket ticket)   //Function added by Tim GU
        {
            _applicationState.NotifyEvent(RuleEventNames.PaymentProcessed,
               new
               {
                   Ticket = ticket,
                   PaymentTypeName = "Aucun Paiement",
                   TenderedAmount = 0,
                   ProcessedAmount = 0,
                   ChangeAmount = 0,
            ...
            ticket.Note = "Aucun Paiement";
            ticket.SetStateValue("Status", "NoPay", "");
            _ticketDao.Save(ticket);

            foreach (var ticketEntity in ticket.TicketEntities)
            {
                string ticketEntityName = ticketEntity.EntityId == 2 ? "Table" : "Customer";
                _applicationState.NotifyEvent(RuleEventNames.EntityUpdated, new
                {
                    EntityTypeId = ticketEntity.EntityTypeId,
                    EntityId = ticketEntity.EntityId,
                    EntityTypeName = ticketEntityName,
                    OpenTicketCount = 0
                });
            }
        ...
        public TicketCommitResult CloseTicket(Ticket ticket)
        {
            ...
            if (canSumbitTicket)
            {
                ...
                if (ticket.Orders.Count > 0)
                {
                    ...
                    if (ticket.Id == 0)
                    {
                        if (ticket.ShouldNotPrint)                //ShouldNotPrint flag check added by Tim GU, to make sure that Cancelled unsubmitted bills wont be saved into DB
                        {
                ...
                if (ticket.Id > 0)// If there is no line in the bill, ID should be 0 here.
                    _ticketDao.Save(ticket);
        ...
        public TicketCommitResult MergeTickets(IEnumerable<int> ticketIds)
        {
            ...
            //if (ticketList.Any(x => x.Calculations.Any()))      //Commented by Tim GU
            ...
            ticketList.ForEach(x => x.RemoveAllCalculations());   //Added by Tim GU
            ...
            var refSRSTransactionNos = string.Join(",", ticketList.Select(x => x.SRSTransactionNo));   //Added by Tim GU

            ticketList.ForEach(x => x.RemoveData());
            ticketList.ForEach(x => CloseTicket(x));

            var ticket = CreateTicket();
            ticket.RefSRSTransactionNo = refSRSTransactionNos;
            ticket.LastModifiedUserName = _applicationState.CurrentLoggedInUser.Name;
        ...
        public TicketCommitResult MoveOrders(Ticket ticket, Order[] selectedOrders, int targetTicketId)
        {
            ...
            var clonedEntites = ticket.TicketEntities.Select(ObjectCloner.Clone).ToList();  //Added by Tim GU
            ...
            clonedEntites.ForEach(x => newTicket.UpdateEntity(x.EntityTypeId, x.EntityId, x.EntityName, x.AccountTypeId, x.AccountId, x.EntityCustomData));  //Added by Tim GU

            RefreshAccountTransactions(newTicket);
            newTicket.LastOrderDate = DateTime.Now;
            newTicket.RefSRSTransactionNo = ticket.SRSTransactionNo;    //Added by Tim GU
            newTicket.LastModifiedUserName = ticket.LastModifiedUserName;   //Added by Tim GU
        ...
        public void CancelSelectedOrders(Ticket ticket)
        {
            ...
            {
                #region SRSCancelOrderBlock

                #endregion   //Region Code Block added by Tim GU, to record the action of cancelling unsubmitted order into the SRS DB.
        ...
        public void CancelSelectedOrder(Ticket ticket, Order order)    //Function added by Tim GU, to record the action of voiding a submitted order into the SRS DB
        {
